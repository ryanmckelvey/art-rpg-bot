...
export async function execute(interaction) {
    ...
    try {
        if ((await getUserById(interaction.user.id)).length > 0) {
            replyMessage = { content: `<@${interaction.user.id}> You're already in the RPG! Use /profile to check yourself out!`, ephemeral: true };
        }
        else {
            ...
            replyMessage = { content: `Welcome <@${interaction.user.id}> to the Art RPG! You have been granted the RPG Player role!`, ephemeral: false };
        }
    }
    catch (error) {
        console.error('Error adding player to RPG database:', error);
        replyMessage = { content: `There was an error adding <@${interaction.user.id}> to the RPG database. Please try again later.`, ephemeral: true };
    }
