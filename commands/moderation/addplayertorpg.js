...
export async function execute(interaction) {
    ...
    try {
        if ((await getUserById(playerToBeAdded.id)).length > 0) {
            replyMessage = { content: `Player <@${playerToBeAdded.id}> is already in the RPG database!`, flags: "Ephemeral" };
        }
        else {
            ...
            replyMessage = { content: `Welcome <@${playerToBeAdded.id}> to the Art RPG! You have been granted the RPG Player role!`};
        }
    }
    catch (error) {
        console.error('Error adding player to RPG database:', error);
        replyMessage = { content: `There was an error adding <@${playerToBeAdded.id}> to the RPG database. Please try again later.`, flags: "Ephemeral" };
    }
